		...
		if($val !='')
		{
			...
			$html = "<table class='tablap'><thead>
				...
				while($row4 = $db->runs($consulta2))
				{
                  ...
                  <td>" . $row4['IMP_PAG'] . "</td>
                  <td>" . $row4['REF_ERE'] . "</td>
                  <td>" . (($row4['REF_ERE']=='0') ? '<a onclick="return confirmCan();" href="?view=cancela&mode=cancelapag&id='.$row4['NUM_PAG'].'"><button class="btn btn-danger cancel btn-xs">Cancelar</button></a>' : ""). "</td>
                  </tr>";
